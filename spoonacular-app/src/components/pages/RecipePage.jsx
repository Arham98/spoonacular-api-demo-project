...
export default function RecipePage({ apiKey }) {
  ...
  const {
    data: dataSimilarRecipes,
    success: successSimilarRecipes,
    loading: loadingSimilarRecipes,
  } = useFetch(
    urlSimilarRecipes,
    optionsStrJSON,
  );
  ...
  const {
    data: dataRecipeWidget,
    success: successRecipeWidget,
    loading: loadingRecipeWidget,
  } = useFetch(
    urlRecipeWidget,
    optionsStrWidget,
  );
      ...
      <Col className="align-items-center">
        <Row style={{ paddingTop: '20px' }}>
          <Col>
            <Row>
              ...
              <div style={{ color: 'white' }}>
                {parse(htmlParser(dataRecipeInfo.summary))}
              </div>
              <p />
              <p style={{ color: 'white' }}>
                {'– By '}
          ...
          <Col className="col-auto">
            <Image
              className="img-fluid"
              src={(dataRecipeInfo.image) ? dataRecipeInfo.image : placeholderImage}
              alt=""
        ...
        <Row>
          <Col style={{ textAlign: 'center' }}>
            <iframe
              title="widget-frame"
              src={`${urlRecipeWidget}&apiKey=${apiKey}`}
              srcDoc={`${
                preScripts[widgetName].join('')
              }<div style="padding-top: 100px;">${
                dataRecipeWidget.replaceAll('hidden', 'visible').replace('width:100%', 'width:30%')}${
